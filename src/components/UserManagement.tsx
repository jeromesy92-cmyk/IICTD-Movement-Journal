...
const UserSkeleton = () => (
  <tr className="animate-pulse border-b border-border">
    <td className="px-6 py-4"><div className="w-4 h-4 bg-muted/50 rounded" /></td>
    ...
    <td className="px-6 py-4"><div className="w-20 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-16 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-24 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-20 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-20 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-20 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-16 h-6 bg-muted/50 rounded-lg" /></td>
    <td className="px-6 py-4"><div className="w-24 h-4 bg-muted/50 rounded" /></td>
    <td className="px-6 py-4"><div className="w-24 h-8 bg-muted/50 rounded-lg ml-auto" /></td>
  </tr>
);
  ...
  <tr>
    <td colSpan={11} className="px-6 py-20 text-center">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mb-6">We couldn't find any users matching your current filters or search criteria.</p>
        <button 
          onClick={onReset}
          className="px-4 py-2 bg-muted/50 border border-border rounded-xl text-sm font-medium text-foreground hover:bg-accent transition-all"
        >
          Clear all filters
        </button>
      </motion.div>
...
const calculatePasswordStrength = (password: string) => {
  ...
  switch (score) {
    ...
    default:
      return { score: 0, label: 'Weak', color: 'bg-muted', textColor: 'text-muted-foreground' };
  }
};
...
export default function UserManagement({ user }: { user: UserData }) {
  ...
  const [availableBaseOffices, setAvailableBaseOffices] = useState<string[]>([]);
  ...
  useEffect(() => {
    fetchUsers();
  }, [currentPage, itemsPerPage, searchTerm, roleFilter, statusFilter, divisionFilter, baseOfficeFilter, sortConfig]);

  const fetchFilters = () => {
    fetch('/api/users/filters')
      .then(res => res.json())
      .then(data => {
        setAvailableDivisions(Array.isArray(data.divisions) ? data.divisions : []);
        setAvailableBaseOffices(Array.isArray(data.baseOffices) ? data.baseOffices : []);
      })
      .catch(err => {
        console.error('Failed to fetch filters:', err);
        setAvailableDivisions([]);
        setAvailableBaseOffices([]);
      });
  };
  ...
  const uniqueDivisions = availableDivisions;
  const uniqueBaseOffices = availableBaseOffices;
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage system access, designations, and personnel assignments.</p>
        </div>
        
        <button 
          onClick={() => setShowForm(true)}
          className="add-user-btn flex items-center gap-2 px-4 py-2 bg-primary rounded-xl text-sm font-medium text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all"
        >
      ...
      <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-2xl ring-1 ring-border">
        <div className="p-5 border-b border-border bg-card/50 flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input 
              type="text" 
              placeholder="Search by name, username or district..." 
              className="w-full bg-muted/50 border border-border rounded-2xl py-2.5 pl-11 pr-4 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all placeholder:text-muted-foreground"
              value={searchTerm}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="relative role-filter">
              <select 
                className="w-full bg-muted/50 border border-border rounded-2xl py-2.5 px-4 pr-10 text-xs text-foreground focus:outline-none focus:border-primary/50 transition-all appearance-none cursor-pointer"
                value={roleFilter}
          ...
          <table className="w-full text-left border-collapse">
              ...
              <tr className="bg-muted/50 text-muted-foreground text-[10px] uppercase tracking-widest font-bold">
                <th className="px-6 py-4 w-12">
                  <div className="relative flex items-center">
                    <input 
                      type="checkbox" 
                      checked={users.length > 0 && selectedUsers.length === (user.role === 'Administrator' ? users.length : users.filter(u => u.role !== 'Administrator').length)}
                ...
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('full_name')}>
                  <div className="flex items-center gap-2">User Details {renderSortIcon('full_name')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('id_number')}>
                  <div className="flex items-center gap-2">ID Number {renderSortIcon('id_number')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('username')}>
                  <div className="flex items-center gap-2">Username {renderSortIcon('username')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('role')}>
                  <div className="flex items-center gap-2">Designation {renderSortIcon('role')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('base_office')}>
                  <div className="flex items-center gap-2">Base Office {renderSortIcon('base_office')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('division')}>
                  <div className="flex items-center gap-2">Division {renderSortIcon('division')}</div>
                </th>
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('district')}>
                ...
                <th className="px-6 py-4 cursor-pointer hover:text-foreground transition-colors" onClick={() => handleSort('last_login')}>
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr key={u.id} className={`hover:bg-muted/50 transition-all group border-b border-border last:border-0 ${selectedUsers.includes(u.id) ? 'bg-primary/5' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="relative flex items-center">
                      <input 
                        type="checkbox" 
                        ...
                        className="peer h-4 w-4 cursor-pointer appearance-none rounded border border-border bg-muted/50 transition-all checked:bg-primary checked:border-primary disabled:opacity-50 disabled:cursor-not-allowed"
                      />
                      <CheckCircle2 className="absolute h-3 w-3 text-primary-foreground opacity-0 peer-checked:opacity-100 left-0.5 pointer-events-none transition-opacity" />
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        ...
                        />
                        <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-card ${u.status === 'active' ? 'bg-emerald-500' : 'bg-slate-500'}`} />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{u.full_name}</p>
                        {u.supervisor_name && (
                  ...
                  <td className="px-6 py-4">
                    <p className="text-xs text-muted-foreground font-mono tracking-tighter">{u.id_number}</p>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2 text-[11px] text-foreground font-semibold">
                      <div className={`w-1.5 h-1.5 rounded-full ${u.role === 'Administrator' ? 'bg-rose-500' : u.role === 'Senior Field Engineer' ? 'bg-amber-500' : 'bg-primary'}`} />
                      <span className="capitalize opacity-80">{u.role}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-xs text-muted-foreground">{u.base_office || 'N/A'}</p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-xs text-muted-foreground">{u.division || 'N/A'}</p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-xs text-muted-foreground truncate max-w-[120px]">{Array.isArray(u.district) ? u.district.join(', ') : (u.district || 'N/A')}</p>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                      u.status === 'active' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-muted/50 text-muted-foreground border border-border'
                    }`}>
                      {u.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {!(user.role !== 'Administrator' && u.role === 'Administrator') && (
                        <>
                          <Tooltip content={u.status === 'active' ? 'Deactivate' : 'Activate'}>
                            <button 
        ...
        {totalUsers > itemsPerPage && (
          <div className="flex items-center justify-between p-6 border-t border-border bg-card/50">
            <div className="text-sm text-muted-foreground">
              Showing <span className="font-bold text-foreground">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-foreground">{Math.min(currentPage * itemsPerPage, totalUsers)}</span> of <span className="font-bold text-foreground">{totalUsers}</span> users
            </div>
      ...
      <Modal
        ...
        title={
          <div>
            <h2 className="text-xl font-black text-foreground tracking-tight">{editingUser ? 'Update Profile' : 'New Account'}</h2>
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1 font-bold">System Access Configuration</p>
          </div>
        }
        size="full"
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all placeholder:text-muted-foreground/50"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">ID Number</label>
                      <input 
                        type="text" 
                        required
                        placeholder="EMP-000"
                        className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 text-sm text-foreground font-mono focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all placeholder:text-muted-foreground/50"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">{editingUser ? 'New Password (Optional)' : 'Initial Password'}</label>
                      <div className="space-y-2">
                        <div className="relative">
                          <input 
                            ...
                            placeholder="••••••••"
                            className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 pr-10 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all placeholder:text-muted-foreground/50"
                          ...
                          <div className="space-y-1.5 px-1">
                                ...
                                return (
                                  <div 
                                    ...
                                    className={`flex-1 rounded-full transition-all duration-300 ${
                                      level <= strength.score ? strength.color : 'bg-muted/30'
                                    }`} 
                                  />
                                );
                              })}
                            ...
                            <div className="flex justify-between items-center">
                              <p className="text-[10px] text-muted-foreground">
                                {formData.password.length < 8 ? 'Must be at least 8 characters' : 'Length requirement met'}
                              </p>
                              <p className={`text-[10px] font-bold uppercase tracking-wider ${calculatePasswordStrength(formData.password).textColor}`}>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        <select 
                          className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 pr-10 text-sm text-foreground focus:outline-none focus:border-primary/50 transition-all appearance-none cursor-pointer"
                          value={formData.role}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 text-sm text-foreground focus:outline-none focus:border-primary/50 transition-all placeholder:text-muted-foreground/50"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Base Office</label>
                      <input 
                        type="text" 
                        required
                        placeholder="e.g. HQ"
                    ...
                    {formData.role === 'Field Engineer' && (
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Assign Supervisor</label>
                        <div className="relative">
                          <select 
                            className="w-full bg-muted/50 border border-border rounded-xl py-2.5 px-4 pr-10 text-sm text-foreground focus:outline-none focus:border-primary/50 transition-all appearance-none cursor-pointer"
                    ...
                    <div className="md:col-span-2 space-y-3">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest ml-1">Assigned Districts</label>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-40 overflow-y-auto p-4 bg-muted/20 border border-border rounded-2xl custom-scrollbar">
                        {ALL_DISTRICTS.map(district => (
                          <label key={district} className="flex items-center gap-2 p-2 hover:bg-muted/50 rounded-lg cursor-pointer transition-colors group">
                            <input 
                              type="checkbox"
                              ...
                              className="w-4 h-4 rounded border-border bg-muted/50 text-primary focus:ring-offset-0 focus:ring-primary"
                            />
                            <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors">{district}</span>
                          </label>
                        ))}
                      </div>
                    </div>
      ...
      <Modal
        ...
        size="full"
        className="bg-[#001a33] border-white/10"
      >
        {viewingUser && (
          <div className="flex flex-col h-full">
            ...
            <div className="px-6 sm:px-10 pb-10 -mt-16 overflow-y-auto custom-scrollbar flex-1">
      ...
      <Modal
        ...
        title={
          <div className="flex flex-col items-center">
            ...
            <h3 className="text-xl font-bold text-white">Confirm Bulk {bulkActionToConfirm === 'delete' ? 'Delete' : 'Action'}?</h3>
          </div>
        }
        size="sm"
      >
