  ...
  const fetchData = async () => {
    ...
    try {
      const [statsRes, divisionRes, districtRes, areaRes, timeRes, usersRes, movementsRes] = await Promise.all([
        ...
        fetch(`/api/movements?limit=10`) // Assuming limit param works or just slice client side if not
      ]);
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Comprehensive insights into field operations and performance.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <select 
            className="bg-[#001a33] border border-white/5 rounded-xl py-2 px-4 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all"
            value={dateRange}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-[#001a33] border border-white/5 rounded-3xl p-8 shadow-xl">
          ...
          <div className="h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#001a33', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topUsers} layout="vertical" margin={{ left: 40 }}>
                ...
                <YAxis dataKey="full_name" type="category" stroke="#64748b" fontSize={10} width={100} />
                <Tooltip cursor={{fill: 'transparent'}} contentStyle={{ backgroundColor: '#001a33', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} />
        ...
        <div className="bg-[#001a33] border border-white/5 rounded-3xl overflow-hidden shadow-xl">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/5">
                {recentMovements.map((m) => (
                  <tr key={m.id} className="text-sm hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4 font-medium text-white">{m.staff_name}</td>
                    <td className="px-6 py-4 text-slate-400 truncate max-w-[200px]">{m.purpose}</td>
                    <td className="px-6 py-4 text-slate-500 font-mono text-xs">{format(new Date(m.date), 'MMM d, yyyy')}</td>
                    <td className="px-6 py-4 text-right">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        m.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400' : 
                        m.status === 'rejected' ? 'bg-rose-500/10 text-rose-400' : 
...
function StatCard({ title, value, icon, trend, color }: any) {
  return (
    <div className="bg-[#001a33] border border-white/5 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`text-xs font-medium ${
            trend.includes('+') ? 'text-emerald-400' : 'text-slate-500'
          }`}>
            {trend}
          </span>
        </div>
